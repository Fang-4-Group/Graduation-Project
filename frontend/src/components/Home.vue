<template>
    <div class="container">
      ...
      <div class="section-boxed">
        ...
        <div class="data-item">
          <label>年齡：</label>
          <span class="data-value">{{ userData.basicInfo.age }}</span>
          <label>睡覺時間：</label>
      ...
      <div class="section-boxed">
        ...
        <div class="furniture-list">
          ...
          <div v-for="(item, index) in userData.housingPreferences.district" :key="index" class="furniture-item">
            {{ item }}
          </div>
        </div>
    </div>
  ...
  onMounted(async () => {
  try {
    const response = await axios.get(`${BASE_URL}/api/getSleepTime/1`); // 假设传入的people_id为1
    sleepTime.value = response.data.sleep_time;
  } catch (error) {
  ...
  .interests-list li {
    ...
    padding: 5px 10px;
    border-radius: 20px;
    background-color: #c2d4dc; /* 莫蘭迪淡藍色 */
    color: #333;
  ...
  .data-value {
    ...
    padding: 5px 10px;
    border-radius: 20px;
    background-color: #d9b9b0; /* 莫蘭迪淡粉色 */
    color: #333;
    display: inline-block;
  }
