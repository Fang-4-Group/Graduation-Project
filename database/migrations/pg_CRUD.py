...
=======
    ...
    async def get_drink_or_smoke(self, people_id: int) -> dict:
        async with self.access_db() as conn:
            try:
                data = await conn.fetchval(
                    """
                    SELECT "Drink_or_Smoke" FROM "PEOPLE" WHERE "People_ID" = $1;
                    """,
                    people_id
                )
                return {"drink_or_smoke": data}
            except Exception as e:
                return {"error": f"Error retrieving drink or smoke habit: {str(e)}"}
        ...
        async with self.access_db() as conn:
            try:
                data = await conn.fetch(
                    """
                    SELECT 
                        "Shopping", "Movie", "Travel", "Music", "Read", "Game", "PE", "Science", "Food"
                    FROM 
                        "PEOPLE" 
                    WHERE 
                        "People_ID" = $1;
                ...
                interests = {key: value for key, value in data[0].items() if value == 1}
                return {"interests": interests}
            except Exception as e:
                return {"error": f"Error retrieving interests: {str(e)}"}

    async def update_sleep_time(self, people_id: int, new_sleep_time: int) -> dict:
        async with self.access_db() as conn:
            try:
                await conn.execute(
                    """
                    UPDATE "PEOPLE" SET "Sleep_Time" = $1 WHERE "People_ID" = $2;
                    """,
    ...
    async def update_drink_or_smoke(self, people_id: int, new_drink_or_smoke: int) -> dict:
        async with self.access_db() as conn:
            try:
                await conn.execute(
                    """
                    UPDATE "PEOPLE" SET "Drink_or_Smoke" = $1 WHERE "People_ID" = $2;
                    """,
                    new_drink_or_smoke, people_id
                )
                return {"message": "Drink or smoke habit updated successfully"}
            except Exception as e:
                return {"error": f"Error updating drink or smoke habit: {str(e)}"}
    ...
    async def add_preference_house_furniture(self, preference_id: int, new_furniture: list) -> dict:
        async with self.access_db() as conn:
            try:
                ...
                return {"message": "Preference house furniture added successfully"}
            except Exception as e:
                return {"error": f"Error adding preference house furniture: {str(e)}"}

    async def add_preference_house_place(self, preference_id: int, new_place: list) -> dict:
        async with self.access_db() as conn:
            try:
                ...
                return {"error": f"Error adding preference house place: {str(e)}"}

    async def update_integer_field(self, column_name: str, people_id: int, new_value: int) -> dict:
        async with self.access_db() as conn:
            try:
                await conn.execute(
                    f"""
                    UPDATE "PEOPLE" SET "{column_name}" = $1 WHERE "People_ID" = $2;
                    """,
                    new_value, people_id
                )
